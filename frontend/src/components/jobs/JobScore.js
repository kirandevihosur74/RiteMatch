...
//     try {
//       const response = await fetch("http://127.0.0.1:8000/api/ai-agent/", {
//         method: "POST",
//         headers: {
...
//         setJobData(data); // optionally replace or merge with existing jobData
//         setOpenDialog(true);
//       } else {
//         const errorData = await response.json();
//         console.error("AI Agent error:", errorData);
//       }
//     } catch (error) {
        ...
        <React.Fragment>
          <Grid container spacing={3}>
            {currentJobs.map((item, index) => {
              const job = item.job || item; // adjust if API format varies
              return (
                <Grid item xs={12} sm={6} md={4} key={index}>
                  <Paper
                    sx={{
